...
RUN conda install bowtie2==2.2.5 -c bioconda -y
RUN conda install tophat==2.1.1 -c bioconda -y
RUN conda install sra-tools==3.0.0 -c bioconda -y

# Required files
WORKDIR /
COPY adapters .
ADD adapters /usr/bin/adapters
...
RUN chmod +x /HAMR/hamr.py && cp /HAMR/hamr.py $BINPATH
